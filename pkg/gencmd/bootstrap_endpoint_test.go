...
func TestBootstrapEndpointsAndResume(t *testing.T) {
	for _, resumed := range []bool{false, true} {
		t.Run(map[bool]string{false: "new", true: "resumed"}[resumed], func(t *testing.T) {
			...
			runBootstrapCommand = func(_ context.Context, args []string) ([]byte, error) {
				out, _, err := runCommand(args, true)
				return []byte(out), err
			}
			...
			runCommand = func(args []string, _ bool) (string, string, error) {
				...
				if args[1] == "etcd" {
					...
					if !strings.Contains(argv, " -e 192.0.2.12") {
						return "", "x509: certificate signed by unknown authority", errors.New("exit status 1")
					}
